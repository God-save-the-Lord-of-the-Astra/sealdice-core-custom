...
/*
	SaveReplyConfig 保存reply（yaml形式）至指定文件

	func SaveReplyConfig(dice *Dice, filename string, newConfig *ReplyConfig) error {
		attrConfigFn := dice.GetExtConfigFilePath("reply", filename)

		// 读取现有的配置文件
		var existingConfig *ReplyConfig
		var err error
		if _, err = os.Stat(attrConfigFn); err == nil {
			existingConfig, _ = CustomReplyConfigRead(dice, filename)
		} else {
			existingConfig = &ReplyConfig{
				Enable:          true,
				Filename:        filename,
				Name:            filename,
				Items:           []*ReplyItem{},
		...
		// 更新现有配置
		existingConfig.Enable = newConfig.Enable
		existingConfig.Conditions = newConfig.Conditions
		existingConfig.Interval = newConfig.Interval

		// 对比并更新Items
		for _, newItem := range newConfig.Items {
			found := false
			for i, existingItem := range existingConfig.Items {
				if ReplyItemsEqual(existingItem, newItem) { // 判断两个ReplyItem是否相等
					existingConfig.Items[i] = newItem
					found = true
					break
				}
			}
			if !found {
				existingConfig.Items = append(existingConfig.Items, newItem)
			}
		}

		// 更新时间戳
		existingConfig.UpdateTimestamp = time.Now().Unix()

		// 保存更新后的配置
		existingConfig.Save(dice)
		return nil
	}

// ReplyItemsEqual 判断两个ReplyItem是否相等

	func ReplyItemsEqual(item1, item2 *ReplyItem) bool {
		if item1.Enable != item2.Enable {
			return false
		}
		if len(item1.Conditions) != len(item2.Conditions) {
			return false
		}
		for i, cond1 := range item1.Conditions {
			if !ReplyConditionsEqual(cond1, item2.Conditions[i]) {
				return false
			}
		}
		if len(item1.Results) != len(item2.Results) {
			return false
		}
		for i, res1 := range item1.Results {
			if !ReplyResultsEqual(res1, item2.Results[i]) {
				return false
			}
		}
		return true
...
// ReplyConditionsEqual 判断两个ReplyConditionBase是否相等

	func ReplyConditionsEqual(cond1, cond2 ReplyConditionBase) bool {
		return reflect.DeepEqual(cond1, cond2)
	}

// ReplyResultsEqual 判断两个ReplyResultBase是否相等

	func ReplyResultsEqual(res1, res2 ReplyResultBase) bool {
